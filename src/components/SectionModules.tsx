...
const MODULES = [
  {
    ...
    icon: <ShieldCheck size={38} className="text-[var(--color-text-dark)]" />,
  },
  {
    ...
    icon: <CalendarDays size={38} className="text-[var(--color-text-dark)]" />,
  },
  {
    ...
    icon: <LayoutDashboard size={38} className="text-[var(--color-text-dark)]" />,
  },
  {
    ...
    icon: <BarChart3 size={38} className="text-[var(--color-text-dark)]" />,
  },
  {
    ...
    icon: <FileText size={38} className="text-[var(--color-text-dark)]" />,
  },
  {
    ...
    icon: <MessageCircle size={38} className="text-[var(--color-text-dark)]" />,
  },
  {
    ...
    icon: <Users size={38} className="text-[var(--color-text-dark)]" />,
  },
  {
    title: 'Vidéo & IA coach (bientôt)',
    desc: 'Analyse vidéo automatisée, suivi IA, dashboards pour coachs et joueurs.',
    icon: <Video size={38} className="text-[var(--color-text-dark)]" />,
  },
];
...
export default function SectionModules() {
  return (
    <section
      id="modules"
      className="w-full bg-[var(--color-text-beige)] py-20 border-t border-[var(--color-text-dark)]/10"
    >
      <div className="max-w-7xl mx-auto px-6">
        <h2 className="text-3xl md:text-5xl font-extrabold text-center mb-12 text-[var(--color-text-dark)] drop-shadow-lg">
          Les <span className="text-[var(--brand-bg)]">modules essentiels</span> SimplyFoot
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {MODULES.map((m) => (
            <div
              key={m.title}
              className="bg-white/60 rounded-2xl p-8 flex flex-col items-center text-center shadow-lg hover:shadow-2xl border border-[var(--color-text-dark)]/15 group transition-all"
            >
              <span className="mb-4">{m.icon}</span>
              <h3 className="text-lg font-extrabold text-[var(--brand-bg)] mb-2 group-hover:text-[var(--color-text-dark)] transition-colors">
                {m.title}
              </h3>
              <p className="text-base text-[var(--color-surface-dark)] font-medium">{m.desc}</p>
            </div>
          ))}
        </div>
      </div>
