...
const HOST = process.env.NEXT_PUBLIC_SITE_URL ?? 'https://www.simplyfoot.fr';
...
export async function generateMetadata({
  ...
  const description = post.excerpt || post.content.replace(/\s+/g, ' ').slice(0, 160);
  ...
  return {
    ...
    openGraph: {
      ...
      title,
      description,
      images: post.cover ? [{ url: post.cover, width: 1200, height: 630, alt: title }] : undefined,
    },
    twitter: {
...
function renderMarkdown(md: string): RenderResult {
  ...
  const flushCode = () => {
    ...
    out.push(
      <pre
        key={`pre-${out.length}`}
        className="rounded-xl bg-[var(--color-surface-dark)] text-white text-sm p-4 overflow-x-auto"
      >
    ...
    while (linkRe.test(rest)) {
      ...
      parts.push(
        <a
          ...
          href={url}
          className="underline decoration-[var(--brand-cta)] hover:text-[var(--brand-cta)]"
  ...
  lines.forEach((raw, i) => {
    ...
    if (line.startsWith('### ')) {
      ...
      out.push(
        <h3
          key={`h3-${i}`}
          id={id}
          className="text-xl font-bold text-[var(--color-text-dark)] mt-6 mb-2 scroll-mt-24"
        >
          {text}
        </h3>,
      );
    ...
    if (line.startsWith('## ')) {
      ...
      out.push(
        <h2
          key={`h2-${i}`}
          id={id}
          className="text-2xl md:text-3xl font-extrabold text-[var(--color-text-dark)] mt-8 mb-3 scroll-mt-24"
        >
          {text}
        </h2>,
      );
    ...
    flushUl();
    out.push(
      <p key={`p-${i}`} className="text-[var(--color-surface-dark)] leading-relaxed">
        {inline(line)}
      </p>,
    );
  });
...
export default async function BlogArticle({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="max-w-6xl mx-auto px-6 py-14">
      ...
      {/* Fil d’Ariane */}
      <nav className="text-sm text-[var(--color-text-beige)]/80 mb-4">
      ...
      <header className="mb-8">
        <h1 className="text-3xl md:text-5xl font-extrabold text-[var(--brand-cta)] drop-shadow">
          {post.title}
        </h1>
        <div className="mt-3 flex flex-wrap items-center gap-3 text-[var(--color-text-beige)]">
        ...
        {post.tags?.length ? (
          <div className="mt-3 flex flex-wrap gap-2">
            {post.tags.map((t) => (
              <span
                key={t}
                className="rounded-full border border-[var(--brand-cta)]/30 bg-[var(--brand-surface)]/50 px-3 py-1 text-xs font-semibold text-[var(--color-text-beige)]"
              >
                #{t}
              </span>
            ))}
      ...
      {post.cover && (
        <div className="mb-8 overflow-hidden rounded-2xl border border-[var(--brand-cta)]/20 shadow">
          <Image
            src={post.cover}
            alt={post.title}
            width={1600}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[260px_minmax(0,1fr)] gap-10">
        {/* TOC */}
        {toc.length ? (
          <aside className="lg:sticky lg:top-24 h-max rounded-2xl border border-[var(--brand-cta)]/20 bg-[var(--brand-surface)]/40 p-4">
            <div className="text-[var(--brand-cta)] font-bold mb-2">Sommaire</div>
            <ul className="space-y-1 text-sm text-[var(--color-text-beige)]">
              {toc.map((i) => (
        ...
        {/* Article */}
        <article className="prose prose-lg max-w-none prose-headings:text-[var(--color-text-dark)] prose-li:marker:text-[var(--brand-cta)] prose-a:text-[var(--color-text-dark)]">
          {nodes}
        </article>
      </div>
      ...
      <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-6">
        {prev ? (
          <Link
            href={`/blog/${prev.slug}`}
            className="group rounded-2xl border border-[var(--brand-cta)]/20 bg-[var(--brand-surface)]/40 p-5 hover:border-[var(--brand-cta)]/40 transition"
          >
            <div className="text-xs text-[var(--color-text-beige)]/80">Article précédent</div>
            <div className="mt-1 font-bold text-white group-hover:text-[var(--brand-cta)]">
              {prev.title}
            </div>
          </Link>
        ) : (
        ...
        {next ? (
          <Link
            href={`/blog/${next.slug}`}
            className="group rounded-2xl border border-[var(--brand-cta)]/20 bg-[var(--brand-surface)]/40 p-5 text-right hover:border-[var(--brand-cta)]/40 transition"
          >
            <div className="text-xs text-[var(--color-text-beige)]/80">Article suivant</div>
            <div className="mt-1 font-bold text-white group-hover:text-[var(--brand-cta)]">
              {next.title}
            </div>
          </Link>
      ...
      <div className="mt-10 flex flex-wrap gap-4 justify-between items-center">
        <Link
          href="/blog"
          className="rounded-xl border border-[var(--brand-cta)]/30 bg-[var(--brand-surface)]/50 px-4 py-2 text-sm text-[var(--color-text-beige)] hover:text-white"
        >
          ← Revenir au blog
        </Link>
        <div className="text-sm text-[var(--color-text-beige)]">
          Partager :
          <a
