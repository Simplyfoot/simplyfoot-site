...
function analyzeCodebase(options = {}) {
    ...
    for (const sourceDir of SOURCE_DIRS) {
        ...
        if (!fs.existsSync(dirPath)) {
            if (verbose) {
                console.warn(`Directory src/${sourceDir} does not exist, skipping.`);
            }
            continue;
        }
        ...
        const processFile = (file, extType) => {
            ...
            if (firstSeg && firstSeg !== relToSourceDir && !isIgnoredSubdir(firstSeg)) {
        ...
        for (const file of findFiles(dirPath, TS_EXTENSIONS)) {
            processFile(file, 'TS');
        }
        for (const file of findFiles(dirPath, TSX_EXTENSIONS)) {
            processFile(file, 'TSX');
        }
    }
    ...
    const tsPercentage =
        totalCodeLines > 0 ? ((tsLines / totalCodeLines) * 100).toFixed(2) : '0.00';
    const tsxPercentage =
